...
void GradientChecker<Dtype>::CheckGradientSingle(Layer<Dtype>* layer,
  ...
  LOG(INFO) << "compute gradient, forwarding";
  layer->Forward(bottom, top);
  // Get additional loss from the objective
  GetObjAndGradient(*layer, top, top_id, top_data_id);
  LOG(INFO) << "compute gradient, backwarding";
  ...
  for (int blob_id = 0; blob_id < blobs_to_check.size(); ++blob_id) {
    ...
    for (int feat_id = 0; feat_id < current_blob->count(); ++feat_id) {
      ...
      if (!element_wise || (feat_id == top_data_id)) {
        ...
        Caffe::set_random_seed(seed_);
        LOG(INFO) << "estimate gradient, positive forwarding";
        layer->Forward(bottom, top);
        positive_objective =
            GetObjAndGradient(*layer, top, top_id, top_data_id);
        ...
        Caffe::set_random_seed(seed_);
        LOG(INFO) << "estimate gradient, negative forwarding";
        layer->Forward(bottom, top);
        negative_objective =
            GetObjAndGradient(*layer, top, top_id, top_data_id);
